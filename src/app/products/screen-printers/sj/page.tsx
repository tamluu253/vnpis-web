...
export const metadata = {
  title: 'Catalog Máy In Lụa SJ | VNPIS',
  description: 'Danh mục chi tiết toàn bộ các model máy in lụa (Screen Printers) của hãng SJ.',
};
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-slate-200 mb-10 flex flex-col md:flex-row items-center justify-between">
          <div className="max-w-2xl mb-8 md:mb-0">
            ...
            <h1 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4">Danh Mục Máy In Lụa SJ</h1>
            <p className="text-lg text-slate-600">
              Tổng hợp các model máy in lụa (Screen Printers) chất lượng cao từ hãng SJ. Đáp ứng mọi nhu cầu từ in lụa phẳng, in lụa tròn đến hệ thống servo tự động chính xác cao.
            </p>
          </div>
          ...
          {sjScreenPrinters.map((machine) => (
            <div key={machine.model} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col overflow-hidden group">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                {/* Chỉ hiển thị các thông số nếu có dữ liệu, vì dòng này hiện tại chưa có thông số */}
                {(machine.plateSize || machine.printArea || machine.speed) && (
                  <div className="space-y-2 mt-auto pt-4 border-t border-slate-100">
                    ...
                    {machine.printArea && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Kích thước hình in:</span>
