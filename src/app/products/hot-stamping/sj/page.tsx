...
export default function SJHotStampingCatalog() {
  return (
    <main className="min-h-screen pt-24 pb-16 bg-slate-50">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {/* Breadcrumb */}
        <div className="mb-8">
          <Link href="/products/hot-stamping" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Trở lại danh mục Máy Ép Nhũ
          </Link>
        </div>
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-slate-200 mb-10 flex flex-col md:flex-row items-center justify-between">
          <div className="max-w-2xl mb-8 md:mb-0">
            ...
            <h1 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4">Danh Mục Máy Ép Nhũ SJ</h1>
            <p className="text-lg text-slate-600">
              Tổng hợp các model máy ép nhũ (Hot Stamping) chất lượng cao từ hãng SJ. Giải pháp hoàn hảo cho việc in ép kim, ép nhiệt bề mặt sản phẩm.
            </p>
          </div>
          ...
          {sjHotStamping.map((machine) => (
            <div key={machine.model} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col overflow-hidden group">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                {(machine.plateSize || machine.printArea || machine.speed) && (
                  <div className="space-y-2 mt-auto pt-4 border-t border-slate-100">
                    {machine.plateSize && machine.plateSize !== 'N/A' && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Kích thước khung:</span>
                        <span className="font-semibold text-slate-700">{machine.plateSize}</span>
                      </div>
                    )}
                    {machine.printArea && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Khu vực ép nhũ:</span>
